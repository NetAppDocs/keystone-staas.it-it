...
.Fasi
. Scaricare il file OVA da https://keystone.netapp.com/downloads/KeystoneCollector-latest.ova[] E memorizzarlo sul sistema VMware vSphere.
...
. Al _passaggio 4: Verifica dei dettagli_, verifica la correttezza e l'autenticità del file OVA. Il nome del prodotto dovrebbe essere _NetApp Keystone Collector_.image:ova-deploy.png["UI OVA"]VCenter convalida l'integrità del contenuto del file OVA e fornisce un digest valido per la firma del codice per i file contenuti nel file OVA. Tuttavia, non convalida l'autenticità del certificato con firma del codice. Per verificare l'integrità, devi scaricare il certificato digest completo della firma e verificarlo rispetto al certificato pubblico pubblicato da Keystone. Attenersi alla seguente procedura:
+
.. Fare clic sul collegamento *Publisher* per scaricare il certificato di digest completo della firma.
.. Scarica il certificato pubblico _Keystone Billing_: https://keystone.netapp.com/downloads/OVA-SSL-NetApp-Keystone-20221101.pem[].
