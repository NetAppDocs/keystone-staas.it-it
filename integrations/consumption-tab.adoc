...
+
image:consumption-trend-burst.png["schermata che mostra i consumi storici"]
...
** *Capacità di burst*: Il limite massimo di burst consentito per il tuo abbonamento.
...
Selezionando un intervallo di date che non termina con la data odierna, le previsioni vengono automaticamente disabilitate. Per riattivarle, viene visualizzata una finestra di dialogo di conferma che informa l'utente che i dati di forecast sono disponibili solo quando l'intervallo di tempo selezionato termina con la data odierna. Confermando, la visualizzazione viene ripristinata all'intervallo predefinito, ovvero gli ultimi 30 giorni di dati storici più i successivi 120 giorni di forecast.

Se si prevede che il consumo previsto superi la capacità di burst, sul grafico compare un indicatore di allarme che consiglia un upgrade della capacità.
