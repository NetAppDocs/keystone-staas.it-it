...
NOTE: Il consumo di capacità negli abbonamenti Keystone viene visualizzato in TiBs sulle dashboard e nei report, ed è arrotondato a due cifre decimali. Se l'utilizzo è inferiore a 0,01 TiB, il valore viene visualizzato come 0 o Nessun utilizzo. I dati in queste schermate vengono visualizzati in orario UTC (fuso orario del server). Quando si inserisce una data per la query, questa viene automaticamente considerata in orario UTC. Per ulteriori informazioni sulle metriche di utilizzo, consultare link:../concepts/metrics.html["Misurazione delle metriche"]. Per informazioni sulle diverse capacità utilizzate in Keystone, vedere link:../concepts/supported-storage-capacity.html["Tipi di capacità di archiviazione in Keystone"].
